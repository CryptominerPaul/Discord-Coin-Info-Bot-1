...
function get_ticker(ticker) {
    return new Promise((resolve, reject) => {
        ...
        switch (exchange.toLowerCase()) {
            case "zeonexchange_btc": {
                exdata.link = `https://exchange.zeonhexalgo.fun/market/63`;
                js_request(`https://exchange.zeonhexalgo.fun/page/api?method=singlemarket&marketid=63` , res => exdata.fillj(res["return"][0], "lasttradeprice", "volume24h", "buy", "sell", ""));
                break;
            }
            case "zeonexchange_ltc": {
                exdata.link = `https://exchange.zeonhexalgo.fun/market/64`;
                js_request(`https://exchange.zeonhexalgo.fun/page/api?method=singlemarket&marketid=64` , res => exdata.fillj(res["return"][0], "lasttradeprice", "volume24h", "buy", "sell", ""));
                break;
            }
           case "crex24": {
                exdata.link = `https://crex24.com/exchange/${coin_up[0]}-${coin_up[1]}`;
                js_request(`https://api.crex24.com/v2/public/tickers?instrument=${coin_up[0]}-${coin_up[1]}`, res => exdata.fillj(res[0], "last", "baseVolume", "bid", "ask", "percentChange"));
                break;
            }
            case "graviex": {
            ...
            case "ihostmn_buy&sell": {
                exdata.link = `https://ihostmn.com/buysell.php?market=${coin_up[0]}${coin_up[1]}`;
                js_request(`https://ihostmn.com/api/v1/buysell/public/get_market_info?market=${coin_up[0]}${coin_up[1]}`, res => {
            ...
            case "southxchange": {
                ...
                js_request(`https://www.southxchange.com/api/price/${coin_up[0]}/${coin_up[1]}`, res => exdata.fillj(res, "Last", "Volume24Hr", "Ask", "Bid", "Variation24Hr"));
                break;
            }
            case "exrates": {
...
client.on("message", msg => {
    ...
    if (  !msg.content.startsWith(conf.prefix) ||  msg.author.bot )
...
//      if ( !msg.content.startsWith(conf.prefix) )
    ...
    switch (args[0]) {
        ...
        case "whattomine": { // easter egg
            msg.channel.send(simple_message("You can NOT mine a POS and MN only coin", "MoneyByte", conf.color.about));
            break;
        }
        case "pos": { // easter egg
            msg.channel.send(simple_message("**MoneyByte**", "**CryptoCoin **1st to Hedge other coins", conf.color.price));
            break;
        }
